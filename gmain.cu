...
int main() {
    ...
    cudaMallocManaged(&test, sizeof(board_str) * 100000);
    cudaMallocManaged(&result, sizeof(board_str) * 6500000);

    printf("Initializing boards\n");
    board template_board = create_board_cuda(1, 8, 8);
    for(size_t i = 0; i < 100000; i++) { 
        printf("\r%lu/1000000", i + 1);
        cudaMallocManaged(&test[i].board, sizeof(uint8_t) * (64 >> 2));
        memset(test[i].board, 0, sizeof(uint8_t) * (64 >> 2));
        if(!test[i].board) err(1, "Memory Error while allocating board's board array\n");

        clone_into_board_cuda(template_board, &test[i]); 
    }
    printf("\n");
    for(size_t i = 0; i < 6500000; i++) { 
        printf("\r%lu/6500000", i + 1);
    ...
    compute_mass_next_cuda<<<16, 1024>>>(test, result, 100000, pitch);
    cudaDeviceSynchronize();
    printf("Cuda Error %d [%s] should be %d\n", cudaPeekAtLastError(), cudaGetErrorName(cudaPeekAtLastError()), cudaSuccess);

    printf("Finished processing\n");

    size_t count = 0;
    for(size_t bs = 0; bs < 100000; bs++) {
